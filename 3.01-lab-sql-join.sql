...
SELECT p.staff_id, s.first_name, s.last_name, SUM(p.amount) as 'Total_Amount', p.payment_date FROM sakila.payment as p
join staff as s
where payment_date > '2005/07/31' and payment_date < '2005/09/00' 
group by staff_id;
...
select p.customer_id, c.last_name, c.first_name,  SUM(p.amount) as 'total_amount'from payment as p
join rental as r
